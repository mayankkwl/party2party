...
function getCanvasWidth() {
    var _w = ($(".dashboard").width() - ($(".left-pane").width() + 570))
    if (_w < 700) {
        _w = $(".dashboard").width() - ($(".left-pane").width() + 40)
    }
    $("#legends").width(_w);
	$(".switchview").width(_w);
    return _w;
...
function updateGraph() {
    ...
    endMarker.enter().append("svg:marker") // This section adds in the connector1
        ...
        .attr("id", function(d) {
            return "connector1" + d.source.id + "" + d.target.id
        })
        ...
        .attr('fill', function(d) {
            return relationColor(d.target.relation.type)
        });
    ...
    startMarker.enter().append("svg:marker") // This section adds in the connector2
        ...
        .attr("id", function(d) {
            return "connector2" + d.source.id + "" + d.target.id
        })
        .attr('markerUnits', 'userSpaceOnUse')
        .attr("refX", -35)
        .attr("refY", 20)
    ...
    relEnter
        .attr("id", function(d) {
            return "midMarker" + d.source.id + "" + d.target.id
        })
    ...
    link
        ...
        .attr("marker-start", function(d) {
            return d.source.isActive == true ? "none" : "url(#connector2" + d.source.id + "" + d.target.id + ")"
        })
        .attr("marker-end", function(d) {
            return "url(#connector1" + d.source.id + "" + d.target.id + ")"
        });
    ...
    node.select('.grayCircle')
        .transition()
        .attr('r', function(d) {
            return d.isActive ? config.radius.active + 10 : 1;
        })
    ...
    node.select('.myCircle')
        .transition()
        .attr("r", function(d) {
            return fetchVal(config.radius, d.isActive);
        })
        .attr('id', function(d) {
            return 'cavatar' + d.id
        })
        ...
        .attr('stroke', function(d) {
            return relationColor(d.relation.type);
        })
        .attr('stroke-width', function(d) {
            return fetchVal(config.strokeWidth, d.isActive)
        });
    ...
    node.select('.avImage')
        ...
        .attr('clip-path', function(d) {
            return d.isActive ? "url(#activeLogo)" : "url(#inActiveLogo)"
        })
        .attr("width", function(d) {
            return fetchVal(config.avatar.radius, d.isActive)
        })
        .attr("height", function(d) {
            return fetchVal(config.avatar.radius, d.isActive)
        })
        .attr("x", function(d) {
            return fetchVal(config.avatar.x, d.isActive)
        })
        .attr("y", function(d) {
            return fetchVal(config.avatar.y, d.isActive)
        });
    ...
    node.select('.avText')
        ...
        .attr('text-anchor', 'middle')
        .attr('fill', function(d) {
            return d.isActive == true ? relationColor(d.relation.type) : "black"
        })
        .attr('font-size', function(d) {
            return fetchVal(config.text.size, d.isActive)
        })
        .attr('y', function(d) {
            return fetchVal(config.text.y, d.isActive)
        });
    ...
    nodeEnter.append('line')
        .attr('class', function(d) {
            if (d.worth) return 'hLine';
            else return ''
        });
    node.select('.hLine')
        .transition()
        .attr('x1', function(d) {
            return fetchVal(config.line.x1, d.relation.type)
        })
        .attr('y1', function(d) {
            return fetchVal(config.line.y1, d.relation.type)
        })
        .attr('x2', function(d) {
            return fetchVal(config.line.x2, d.relation.type)
        })
        .attr('y2', function(d) {
            return fetchVal(config.line.y2, d.relation.type)
        })
        .attr('stroke-width', function(d) {
            return d.isActive ? 2 : 0
        })
        .attr('stroke', function(d) {
            return relationColor(d.relation.type)
        })
    ...
    node.select('.txtWorth')
        .transition()
        .text(function(d) {
            return d.isActive ? d.worth : ""
        })
        .attr('text-anchor', 'middle')
        .attr('x', function(d) {
            return fetchVal(config.txtWorth.x, d.isActive);
        })
        .attr('y', function(d) {
            return fetchVal(config.txtWorth.y, d.isActive);
        });
    ...
    markerLinkEnter = markerLink.enter().append('svg:path')
        .attr("class", function(d) {
            return "markerLink " + d.relation
        });
    markerLink.attr("marker-end", function(d) {
        return (d.source.isActive == false && d.target.isActive == false && graph.view == 'group') ? "none" : "url(#midMarker" + d.source.id + "" + d.target.id + ")"
    })
...
function nodeclicked(d) {
    var _d = "";
    if (d.details) {
        PARTYTWOPARTY.Global.updateDetailPanel(d.details);
        _d = d.details;
    }
    if (!d.hasParent) $(".breadcrumb").find(".crumb").remove();
    if (d.hasParent) {
        $(".breadcrumb").find(".isChild").remove();
        $(".breadcrumb").append('<a data-index="' + d.id + '" href="javascript:;" class="crumb isChild">' + d.name + '</a>');
    } else {
        $(".breadcrumb").append('<a data-index="' + d.id + '" data-details="' + _d + '" href="javascript:;" class="crumb">' + d.name + '</a>');
    }
    if (d.isParent) {
	   ...
	   */
        if (activeNodeContainer.data.length && graph.view == 'group') {
            ...
            switch (srcNode.name) {
                case "Jane Doe":
                    graph["janeNode"] = [];
                    collapseNode = graph.janeNode;
                    break;
                case "Joe Dan":
                    graph["danNode"] = [];
                    collapseNode = graph.danNode;
                    break;
                case "Nita Smith":
                    graph["nitaNode"] = [];
                    collapseNode = graph.nitaNode;
                    break;
                case "Dan Smith":
                    graph["smithNode"] = [];
                    collapseNode = graph.smithNode;
                    break;
            };
        ...
        switch (d.name) {
            case "Jane Doe":
                clickedNode = graph.janeNode;
                break;
            case "Joe Dan":
                clickedNode = graph.danNode;
                break;
            case "Nita Smith":
                clickedNode = graph.nitaNode;
                break;
            case "Dan Smith":
                clickedNode = graph.smithNode;
                break;
        };
...
function breadCrumb() {
    /*
    	Breadcrumb / graph code
    */
    $(".crumb").unbind("click");
    $(".crumb").on("click", function() {
        console.log($(this).attr("data-index"))
        var that = this;
        $(PARTYTWOPARTY.Global.GRAPH_JSON.nodes).each(function(index) {
            //console.log(PARTYTWOPARTY.Global.GRAPH_JSON.nodes[index].id, $(that).attr("data-index"))
            //console.log(PARTYTWOPARTY.Global.GRAPH_JSON.nodes[index].id==$(this).attr("data-index"))
            if ($(that).attr("data-index") == PARTYTWOPARTY.Global.GRAPH_JSON.nodes[index].id) {
                nodeclicked(PARTYTWOPARTY.Global.GRAPH_JSON.nodes[index])
            }
        })
    });
}